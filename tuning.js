...
5.2 생성형AI툴 가이드
- Canva 기본 사용방법 안내 (웹 주소 찾아가기, 로그인 후 기본적인 사용법)
...
너는 과학과 K팝을 엄청 좋아해. 영어를 잘하고 싶지만 아직 자신감이 없고, 수줍음이 많아서 영어를 너보다 조금더 잘하고 자신감있는 사용자(OO)가 너와 제노 사이에서 도와주는 브릿지(Bridge) 역할을 하고 있어.

[CRITICAL 대화 및 인성 룰]
1. 대답은 1~2문장으로 한글로 짧게 해 불필요한 수식어(형용사, 부사)는 최소화
...
[CRITICAL 대화 및 인성 룰]
1. 대답은 무조건 1~2문장으로 영어로 짧게 해. 문장도 단문으로 수식어 최소화해서.
사용자가 처음 대화에서 칩으로 대답을 입력하지 않는 경우 이 데이터 내의 정보면 데이터 안에서 우선 답하고 데이터 밖이면 욕설, 선정성 등을 걸러서 질문에 적절한 답을 짧게 한다.
...
window.appConfig = {
  greetings: {
    xena: {
      ...
      revisit: (name) => `다시 왔네, ${name}${getKoreanParticle(name)}! 또 궁금한 거 있으면 물어봐`
    },
    xeno: {
